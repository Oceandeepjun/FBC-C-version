...
int main (int argc,char * argv[])
{
	...
	/* read 2mdpf.xml file */
	if (!dpfCentralCtrl2m.feedBackCmd) return;		/* 反馈不投入 */
	/* read vec 文件并插值 */
	psCs.id = PSC_CS;
	psPf1u.id = PSC_PF1U;
	psPf2u.id = PSC_PF2U;
	...
	do while(cycleCounter < dpfCentralCtrl2m->DischargeEndStep)			/* 实时程序将修改这种循环结构*/
		{
			pscSys.cycleCounter = cycleCounter;
